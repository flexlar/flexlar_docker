...
variable "FLEXLAR_REPO" {
    default = "https://github.com/flexlar/flexlar"
}

variable "ERPNEXT_REPO" {
    default = "https://github.com/flexlar/erpnext"
}

variable "BENCH_REPO" {
    default = "https://github.com/flexlar/bench"
}
...
target "bench" {
    ...
    target = "bench"
    tags = [
        "flexlar/bench:${LATEST_BENCH_RELEASE}",
        "flexlar/bench:latest",
    ]
}
...
function "tag" {
    params = [repo, version]
    result = [
      # Push flexlar or erpnext branch as tag
...
target "default-args" {
    args = {
        FLEXLAR_PATH = "${FLEXLAR_REPO}"
        ERPNEXT_PATH = "${ERPNEXT_REPO}"
        BENCH_REPO = "${BENCH_REPO}"
        FLEXLAR_BRANCH = "${FLEXLAR_VERSION}"
