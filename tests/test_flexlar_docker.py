...
def test_endpoints(url: str, callback: Any, flexlar_site: str):
    check_url_content(
        url=f"http://127.0.0.1{url}", callback=callback, site_name=flexlar_site
    )


@pytest.mark.skipif(
    os.environ["FLEXLAR_VERSION"][0:3] == "v12", reason="v12 doesn't have the asset"
)
def test_assets_endpoint(flexlar_site: str):
    check_url_content(
        url=f"http://127.0.0.1/assets/flexlar/images/flexlar-framework-logo.svg",
        callback=assets_cb,
        site_name=flexlar_site,
    )


def test_files_reachable(flexlar_site: str, tmp_path: Path, compose: Compose):
    content = "lalala\n"
    ...
    compose(
        "cp",
        str(file_path),
        f"backend:/home/flexlar/flexlar-bench/sites/{flexlar_site}/public/files/",
    )
...
@pytest.mark.parametrize("service", BACKEND_SERVICES)
@pytest.mark.usefixtures("flexlar_site")
def test_flexlar_connections_in_backends(
    service: str, python_path: str, compose: Compose
):
    filename = "_ping_flexlar_connections.py"
    compose("cp", f"tests/{filename}", f"{service}:/tmp/")
    compose.exec(
        "-w",
...
def test_push_backup(
    ...
    compose.bench("--site", flexlar_site, "backup", "--with-files")
    restic_args = [
        "--env=RESTIC_REPOSITORY=s3:http://minio:9000/flexlar",
...
def test_https(flexlar_site: str, compose: Compose):
    compose("-f", "overrides/compose.https.yaml", "up", "-d")
    check_url_content(url="https://127.0.0.1", callback=index_cb, site_name=flexlar_site)
