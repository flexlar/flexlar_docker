...
def env_file(tmp_path_factory: pytest.TempPathFactory):
    tmp_path = tmp_path_factory.mktemp("flexlar-docker")
    file_path = tmp_path / ".env"
    shutil.copy("example.env", file_path)

    _add_sites_var(file_path)

    for var in ("FLEXLAR_VERSION", "ERPNEXT_VERSION"):
...
def flexlar_setup(compose: Compose):
    compose.stop()
...
@pytest.fixture(scope="session")
def flexlar_site(compose: Compose):
...
@pytest.fixture
def python_path():
    return "/home/flexlar/flexlar-bench/env/bin/python"
