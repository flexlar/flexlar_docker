...
This setup is a very simple single compose file that does everything to start required services and a flexlar-bench. It is used to start play with docker instance with a site. The file is located in the root of repo and named `pwd.yml`.

## Services

### flexlar-bench components
...
- create-site, creates one site to serve as default site for the flexlar-bench.

### Service dependencies

- db, mariadb, container with flexlar specific configuration.
...
This serves only site called `frontend` through the nginx. `FLEXLAR_SITE_NAME_HEADER` is set to `frontend` and a default site called `frontend` is created.
