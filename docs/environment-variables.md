...
- `FLEXLAR_SITE_NAME_HEADER`: Set proxy header `X-Flexlar-Site-Name` and serve site named in the header, defaults to `$host`, i.e. find site name from host header. More details [below](#flexlar_site_name_header)
...
Flexlar framework release. You can find all releases [here](https://github.com/flexlar/flexlar/releases).
...
ERPNext [release](https://github.com/flexlar/erpnext/releases). This variable is required if you use ERPNext override.

### `LETSENCRYPT_EMAIL`

Email that used to register https certificate. This one is required only if you use HTTPS override.

### `FLEXLAR_SITE_NAME_HEADER`
