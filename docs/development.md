...
Clone and change directory to flexlar_docker directory
...
For most people getting started with Flexlar development, the best solution is to use [VSCode Dev Containers extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers).
...
- Open flexlar_docker folder in VS Code.
  - `code .`
...
- The `development` directory is ignored by git. It is mounted and available inside the container. Create all your benches (installations of bench, the tool that manages flexlar) inside this directory.
...
NOTE: Prior to doing the following, make sure the user is **flexlar**.

```shell
bench init --skip-redis-config-generation flexlar-bench
cd flexlar-bench
```

To setup flexlar framework version 14 bench set `PYENV_VERSION` environment variable to `3.10.5` (default) and use NodeJS version 16 (default),

```shell
# Use default environments
bench init --skip-redis-config-generation --flexlar-branch version-14 flexlar-bench
# Or set environment versions explicitly
nvm use v16
PYENV_VERSION=3.10.13 bench init --skip-redis-config-generation --flexlar-branch version-14 flexlar-bench
# Switch directory
cd flexlar-bench
```

To setup flexlar framework version 13 bench set `PYENV_VERSION` environment variable to `3.9.17` and use NodeJS version 14,

```shell
nvm use v14
PYENV_VERSION=3.9.17 bench init --skip-redis-config-generation --flexlar-branch version-13 flexlar-bench
cd flexlar-bench
```
...
Honcho is the tool used by Bench to manage all the processes Flexlar requires. Usually, these all run in localhost, but in this case, we have external containers for Redis. For this reason, we have to stop Honcho from trying to start Redis processes.

Honcho is installed in global python environment along with bench. To make it available locally you've to install it in every `flexlar-bench/env` you create. Install it using command `./env/bin/pip install honcho`. It is required locally if you wish to use is as part of VSCode launch configuration.
...
This will create a new site and a `development.localhost` directory under `flexlar-bench/sites`.
...
To develop a new app, the last step will be setting the site into developer mode. Documentation is available at [this link](https://flexlar.io/docs/user/en/guides/app-development/how-enable-developer-mode-in-flexlar).
...
Note: Both flexlar and erpnext must be on branch with same name. e.g. version-14
...
### Start Flexlar without debugging

Execute following command from the `flexlar-bench` directory.
...
usage: installer.py [-h] [-j APPS_JSON] [-b BENCH_NAME] [-s SITE_NAME] [-r FLEXLAR_REPO] [-t FLEXLAR_BRANCH] [-p PY_VERSION] [-n NODE_VERSION] [-v] [-a ADMIN_PASSWORD] [-d DB_TYPE]
...
options:
  ...
  -b BENCH_NAME, --bench-name BENCH_NAME
                        Bench directory name, default: flexlar-bench
  -s SITE_NAME, --site-name SITE_NAME
                        Site name, should end with .localhost, default: development.localhost
  -r FLEXLAR_REPO, --flexlar-repo FLEXLAR_REPO
                        flexlar repo to use, default: https://github.com/flexlar/flexlar
  -t FLEXLAR_BRANCH, --flexlar-branch FLEXLAR_BRANCH
                        flexlar repo to use, default: version-15
...
### Start Flexlar with Visual Studio Code Python Debugging
...
- Click on `Install on Dev Container: Flexlar Bench`
- Click on 'Reload'

We need to start bench separately through the VSCode debugger. For this reason, **instead** of running `bench start` you should run the following command inside the flexlar-bench directory:
...
Launch VSCode command palette (cmd+shift+p or ctrl+shift+p), run the command `Python: Select interpreter to start Jupyter server` and select `/workspace/development/flexlar-bench/env/bin/python`.

The first step is installing and updating the required software. Usually the flexlar framework may require an older version of Jupyter, while VSCode likes to move fast, this can [cause issues](https://github.com/jupyter/jupyter_console/issues/158). For this reason we need to run the following command.

```shell
/workspace/development/flexlar-bench/env/bin/python -m pip install --upgrade jupyter ipykernel ipython
```
...
flexlar.init(site='development.localhost', sites_path='/workspace/development/flexlar-bench/sites')
flexlar.connect()
flexlar.local.lang = flexlar.db.get_default('lang')
flexlar.db.connect()
```
...
```shell
docker exec -e "TERM=xterm-256color" -w /workspace/development -it devcontainer-flexlar-1 bash
```
...
Access the service by service name from the `flexlar` development container. The above service will be accessible via hostname `postgresql`. If ports are published on to host, access it via `localhost:5432`.
