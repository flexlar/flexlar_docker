...
if [[ -z "$FLEXLAR_SITE_NAME_HEADER" ]]; then
  # shellcheck disable=SC2016
  echo 'FLEXLAR_SITE_NAME_HEADER defaulting to $host'
  # shellcheck disable=SC2016
  export FLEXLAR_SITE_NAME_HEADER='$host'
fi
...
envsubst '${BACKEND}
  ...
  </templates/nginx/flexlar.conf.template >/etc/nginx/conf.d/flexlar.conf
